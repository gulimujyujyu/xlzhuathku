...
namespace
{
    ...
    fbxDouble3 GetMaterialProperty(const KFbxSurfaceMaterial * pMaterial,
        ...
        GLuint & pTextureName)
    {
        ...
        if (lProperty.IsValid() && lFactorProperty.IsValid())
        {
            ...
            double lFactor = KFbxGet<double>(lFactorProperty);
            if (lFactor != 1)
            {
                lResult[0] *= lFactor;
                lResult[1] *= lFactor;
...
VBOMesh::~VBOMesh()
{
    ...
    FbxSdkDeleteAndClear(mSubMeshes);
}

bool VBOMesh::Initialize(const KFbxMesh *pMesh)
{
    if (!pMesh->GetNode())
        return false;
...
void VBOMesh::UpdateVertexPosition(const KFbxMesh * pMesh, const KFbxVector4 * pVertices) const
{
...
void VBOMesh::EndDraw(ShadingMode pShadingMode) const
{
    // Reset VBO binding.
...
bool MaterialCache::Initialize(const KFbxSurfaceMaterial * pMaterial)
{
    const fbxDouble3 lEmissive = GetMaterialProperty(pMaterial,
        KFbxSurfaceMaterial::sEmissive, KFbxSurfaceMaterial::sEmissiveFactor, mEmissive.mTextureName);
    ...
    const fbxDouble3 lAmbient = GetMaterialProperty(pMaterial,
        KFbxSurfaceMaterial::sAmbient, KFbxSurfaceMaterial::sAmbientFactor, mAmbient.mTextureName);
    ...
    const fbxDouble3 lDiffuse = GetMaterialProperty(pMaterial,
        KFbxSurfaceMaterial::sDiffuse, KFbxSurfaceMaterial::sDiffuseFactor, mDiffuse.mTextureName);
    ...
    const fbxDouble3 lSpecular = GetMaterialProperty(pMaterial,
        KFbxSurfaceMaterial::sSpecular, KFbxSurfaceMaterial::sSpecularFactor, mSpecular.mTextureName);
    ...
    KFbxProperty lShininessProperty = pMaterial->FindProperty(KFbxSurfaceMaterial::sShininess);
...
LightCache::LightCache() : mType(KFbxLight::ePOINT)
{
...
bool LightCache::Initialize(const KFbxLight * pLight, KFbxAnimLayer * pAnimLayer)
{
    mType = pLight->LightType.Get();

    KFbxTypedProperty<fbxDouble3> lColorProperty = pLight->Color;
    ...
    mColorRed.mAnimCurve = lColorProperty.GetCurve<KFbxAnimCurve>(pAnimLayer, KFCURVENODE_COLOR_RED);
    mColorGreen.mAnimCurve = lColorProperty.GetCurve<KFbxAnimCurve>(pAnimLayer, KFCURVENODE_COLOR_GREEN);
    mColorBlue.mAnimCurve = lColorProperty.GetCurve<KFbxAnimCurve>(pAnimLayer, KFCURVENODE_COLOR_BLUE);

    if (mType == KFbxLight::eSPOT)
    {
        KFbxTypedProperty<fbxDouble1> lConeAngleProperty = pLight->HotSpot;
        mConeAngle.mValue = static_cast<GLfloat>(lConeAngleProperty.Get());
        mConeAngle.mAnimCurve = lConeAngleProperty.GetCurve<KFbxAnimCurve>(pAnimLayer);
    }

    return true;
}

void LightCache::SetLight(const KTime & pTime) const
{
    ...
    if (mType == KFbxLight::eDIRECTIONAL)
    {
    ...
    if (mType == KFbxLight::eSPOT && lConeAngle != 0.0)
    {
...
void LightCache::IntializeEnvironment(const KFbxColor & pAmbientLight)
{
