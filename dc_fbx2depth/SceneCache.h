...
public:
    ...
    bool Initialize(const KFbxMesh * pMesh);

    // Update vertex positions for deformed meshes.
    void UpdateVertexPosition(const KFbxMesh * pMesh, const KFbxVector4 * pVertices) const;
    ...
    void EndDraw(ShadingMode pShadingMode) const;
...
private:
    ...
    KArrayTemplate<SubMesh*> mSubMeshes;
    bool mHasNormal;
    bool mHasUV;
...
public:
    ...
    bool Initialize(const KFbxSurfaceMaterial * pMaterial);
...
struct PropertyChannel
{
    ...
    GLfloat Get(const KTime & pTime) const
    {
        if (mAnimCurve)
        {
...
public:
    ...
    static void IntializeEnvironment(const KFbxColor & pAmbientLight);

    bool Initialize(const KFbxLight * pLight, KFbxAnimLayer * pAnimLayer);

    // Draw a geometry (sphere for point and directional light, cone for spot light).
    // And set light attributes.
    void SetLight(const KTime & pTime) const;

private:
    static int sLightCount;         // How many lights in this scene.

    GLuint mLightIndex;
    KFbxLight::ELightType mType;
